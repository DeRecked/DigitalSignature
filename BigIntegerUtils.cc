...
BigUnsigned stringToBigUnsigned_base16(const std::string &s) {
...
BigInteger stringToBigInteger_base16(const std::string &s) {
	// Recognize a sign followed by a BigUnsigned.
	return (s[0] == '-') ? BigInteger(stringToBigUnsigned_base16(s.substr(1, s.length() - 1)), BigInteger::negative)
		: (s[0] == '+') ? BigInteger(stringToBigUnsigned_base16(s.substr(1, s.length() - 1)))
		: BigInteger(stringToBigUnsigned_base16(s));
}
